...
public class ContainersRealmController {
    ...
    public static void deleteFromRealmAllContainers() {
        for (RealmFoldersContainer realmFoldersContainer: RealmDb.realm().where(RealmFoldersContainer.class).findAll()){

            for (FolderNotesObject folderNotesObject : RealmDb.realm().where(FolderNotesObject.class).findAll()){
                long id = folderNotesObject.getId();
                FolderNotesRealmController.delFolderNote(id);
            }

            for (FolderTaskObject folderTaskObject: RealmDb.realm().where(FolderTaskObject.class).findAll()){
                FolderTaskRealmController.deleteFolder(folderTaskObject);
            }

            for (ReportObject reportObject:RealmDb.realm().where(ReportObject.class).findAll()){
                ReportRealmController.delReport(reportObject.getId());
            }

        }

        RealmDb.realm().where(RealmFoldersContainer.class).findAll().deleteAllFromRealm();
    }
}
