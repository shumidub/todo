...
**File changed:** `app/src/main/java/com/shumidub/todoapprealm/ui/fragment/task_section/small_tasks_fragment/SmallTasksFragment.java`

**Diff (method `notifyDataChanged()`, lines 252-276):**
...
Новая строка вставлена внутри существующей `else`-ветки, поэтому null-guard на строке 259 уже покрывает её (адаптер гарантированно не null в этой ветке). В ветке `null` конструктор `TasksRecyclerViewAdapter` сам зовёт `rebuildItems()` (строка 118 адаптера), снапшот консистентен.

**Build status:** `./gradlew assembleDebug` → `BUILD SUCCESSFUL in 18s`, 35 actionable tasks, никаких ошибок/warning'ов кроме штатного realm-mediator unchecked notice.

**Side-effect note:** `setTasksAndNotifyDataSetChanged()` (строки 204-212) теперь будет вызывать `rebuildItems()` дважды — один раз напрямую (строка 209), второй раз через `notifyDataChanged()`. Операция идемпотентна (читает текущее состояние `tasks` + `sections`, результат тот же), оверхед — один лишний проход по `tasks` и `sections` на caller. Пренебрежимо мало, упрощать call-site не входит в задачу (см. Risks #3 в Design).

**Call-sites, которые починены автоматически (без правок):**
- `TasksRecyclerViewAdapter.bindTask()` строка ~319 — checkbox-handler.
- `TaskActionModeCallback` строки 156, 282, 299 — delete / categories / edit.

## Review

**Reviewer verdict:** All clean. READY.

**Commits found (master, atomic):**
- `7d03cd1` orchestrator(sprint-002/task-004): requirements draft + diagnosis hints
- `feadb6b` orchestrator(sprint-002/task-004): requirements finalized
- `9940301` orchestrator(sprint-002/task-004): design
- `d5fc462` fix(sprint-002/task-004): rebuild adapter items in notifyDataChanged() — done tasks now hide on checkbox tap
- `7d68a7e` orchestrator(sprint-002/task-004): implementation log

### Requirements compliance
- **R1 / R5 (default-mode hide):** Fix forces `rebuildItems()` → `flatten()` re-reads `tasks` (RealmResults proxy with `done=false` filter when `isAllTaskShowing == false`). Snapshot `items` shrinks → done task disappears from list on checkbox tap. Verified via code path: `bindTask()` checkbox handler → `setTaskDoneOrParticullaryDone` (Realm mutation, committed synchronously) → `notifyDataChanged()` → new `rebuildItems()` line → re-flatten from filtered `tasks` → `notifyDataSetChanged()`. ✓
- **R2 (show-completed unaffected):** `showAllTasks()` does NOT call `notifyDataChanged()` — it constructs a new adapter (lines 283/294) which calls `rebuildItems()` itself. New line on `notifyDataChanged()` path is irrelevant to this toggle. ✓
- **R3 (instant rerender):** `notifyDataChanged()` is the immediate path on checkbox/ActionMode mutations; rebuild + notifyDataSetChanged is synchronous. ✓
- **R4 (all 3 tabs):** Each tab has its own `SmallTasksFragment` instance with its own adapter; fix lives in fragment method, applies identically per-tab. ✓
- **R6/R7:** Fix-point matches the design exactly — `SmallTasksFragment.notifyDataChanged()`, inside the `else` branch, before `notifyDataSetChanged()`. ✓

### Design compliance
- Single-line addition `tasksRecyclerViewAdapter.rebuildItems();` at line 264, immediately before `notifyDataSetChanged()`. ✓
- Inside the `else` branch (line 263) — null-guard from the surrounding `if (tasksRecyclerViewAdapter == null)` (line 259) covers it. ✓
- Verified line 264 in the file matches the design diff verbatim. ✓

### No collateral changes
- `git show --stat d5fc462`: 1 file changed, 1 insertion(+). Only `SmallTasksFragment.java`. No other methods or files touched. ✓

### Atomic commits on master
- All 5 commits are atomic, single-purpose, on `master`. Working tree clean. ✓

### Side effects (per Design Risks #3)
- `setTasksAndNotifyDataSetChanged()` (line 209) now triggers `rebuildItems()` twice — once directly, once via `notifyDataChanged()` at line 211. Confirmed idempotent: `rebuildItems()` is `public void rebuildItems() { items = flatten(); }` (TasksRecyclerViewAdapter line 132-134) which fully replaces the `items` list each call by re-reading the current `tasks` field. No accumulation, no stale state, just one extra full `flatten()` pass. Overhead negligible. ✓

### Edge cases
- **`tasksRecyclerViewAdapter == null` during teardown:** The `else` branch is only entered when adapter is non-null. The `if` branch creates a new adapter (constructor calls `rebuildItems()` internally at line 128). Either branch leaves a consistent snapshot. ✓
- **`tasks` empty:** `flatten()` handles `tasks == null` (line 152) and empty `tasks` (line 154 loop skipped, footer added only if doneTasks non-empty). `rebuildItems()` is safe. ✓
- **`tasks` is RealmResults proxy with closed Realm:** Not introduced by this fix — same risk pre-existed in adapter constructor and `setTasksAndNotifyDataSetChanged()`. All current call-sites of `notifyDataChanged()` are post-Realm-mutation on an open Realm. ✓
- **Infinite recursion:** `rebuildItems()` does not call `notifyDataChanged()`. RecyclerView's `notifyDataSetChanged()` is framework-internal, doesn't call back into the fragment. No cycle. ✓

### Build / verification
- Implementation log records `./gradlew assembleDebug` → `BUILD SUCCESSFUL in 18s`. Trust the log; reviewer did not re-run.

**No issues found. Ready for Phase 7 manual QA.**
